...
public class Pack_3_Streams_Easy {
    ...
    public void exercise_1_map_collect() {
        ...
        Set<Integer> result = numbers.stream().map(number->number/2).collect(toSet());
    ...
    public void exercise_2_map() {
        ...
        List<Integer> result = strings.stream().map(number->Integer.parseInt(number)%13).distinct().sorted().collect(toList());
        System.out.println(result);
    ...
    public void exercise_3_reduce() {
        ...
        int result = 0;
        Optional<String> stringResult=strings.stream().reduce((string1,string2)->string1.length()>string2.length()?string1:string2);
        result=stringResult.get().length();
        System.out.println(result);
...
//        sum = list.stream().sum(); // doesn't compile, not an int stream
    ...
    public void exercise_4_filter_limit() {
        ...
        List<Integer> result = numbers.filter(n->n%17==0).limit(5).boxed().collect(toList());
        System.out.println(result);
