...
public class Pack_4_Streams_Difficult {
    ...
    public void exercise_1_findFirst() {
        ...
        String result= null;
        result= String.valueOf(EMPLOYEES.stream()
                .map(e->e.getFirstName()+" "+e.getSurname())//.peek(System.out::println)
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))
                .entrySet()
                .stream()
                .peek(System.out::println)
                .filter(element -> element.getValue() > 1).map(e->e.getKey()).peek(System.out::println)
                .findFirst().get());
    ...
    public void exercise_2_groupingBy_counting() {
        ...
        long result = 0;
        result= Long.valueOf(EMPLOYEES.stream()
                .map(e->e.getHomeAddress().getPostCode().toString().substring(0,2))//.peek(System.out::println)
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))
                .entrySet()
                .stream()
                .filter(element -> element.getValue() >= 5).map(e->e.getValue())
                .count()
        );
    ...
    public void exercise_3_flatMap() {
        ...
        long result = EMPLOYEES.stream().map(e->e.getCorrespondenceAddress()+" "+e.getHomeAddress()).distinct().count();
        long res2=EMPLOYEES.stream().filter(distinctByKey(e->e.getCorrespondenceAddress()+" "+e.getHomeAddress())).count();
        long res3=Long.valueOf(EMPLOYEES.stream().map(e->e.getCorrespondenceAddress()+" "+e.getHomeAddress())
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))
                .entrySet()
                .stream()
                .count());
        assertThat(res2, sameBeanAs(1820L));//through any method, the result is 1764, not 1820.
    }
    ...
    public void exercise_4_groupingBy_summingInt() {
        ...
        List<String> result = EMPLOYEES.stream()
                .collect(Collectors.groupingBy(Employee::getCompany,Collectors.summingInt(e->e.getSalary().intValue()))).entrySet()
                        .stream().sorted((e1,e2)->e2.getValue()-e1.getValue()).map(e->e.getKey().getName()+" - "+decimalFormat.format(e.getValue())).collect(Collectors.toList());
    ...
    public void exercise_6_mapToLong() {
        ...
        long result = Arrays.stream(rows).mapToLong((e->e* Arrays.stream(columns).sum())).sum();
    ...
    public void exercise_7_randomLongs_concat_toArray() {
        ...
        int[] result =LongStream.concat(longs,ints.asLongStream())
                ...
                .skip(5)
                .limit(10)
                .map(e->e%1000) .mapToInt(e->Integer.valueOf(String.valueOf(e))).toArray();
